...
def getInfo (coin,a,cur): # Function to get the info
...
def lookup():
	...
	for coin in my_portfolio:
		...
		# Do some math
		info=getInfo(coin['sym'],'1','USD')
		x={}
		x['name']=info['data']['name']
		x["price_usd"]=info['data']['quote'][cur]['price']
		...
		profit_loss_per_coin = float(
                    x["price_usd"]) - float(coin["price_paid_per"])
		...
		current_price = Label(root, text="${0:.2f}".format(
                    float(x["price_usd"])), bg="white",)
		current_price.grid(row=row_count, column=2, sticky=N+S+E+W)

		price_paid = Label(root, text="${0:.2f}".format(
                    float(coin["price_paid_per"])), bg="silver")
		price_paid.grid(row=row_count, column=3, sticky=N+S+E+W)

		profit_loss_per = Label(root, text="${0:.2f}".format(float(
                    profit_loss_per_coin)), bg="white", fg=red_green(float(profit_loss_per_coin)))
		profit_loss_per.grid(row=row_count, column=4, sticky=N+S+E+W)

		# one_hr_change = Label(root, text="{0:.2f}%".format(float(
        #             x["percent_change_1h"])), bg="silver", fg=red_green(float(x["percent_change_1h"])))
		# one_hr_change.grid(row=row_count, column=5, sticky=N+S+E+W)

		# tf_hr_change = Label(root, text="{0:.2f}%".format(float(
        #             x["percent_change_24h"])), bg="white", fg=red_green(float(x["percent_change_24h"])))
		# tf_hr_change.grid(row=row_count, column=6, sticky=N+S+E+W)

		# seven_day_change = Label(root, text="{0:.2f}%".format(float(
        #             x["percent_change_7d"])), bg="silver", fg=red_green(float(x["percent_change_7d"])))
		# seven_day_change.grid(row=row_count, column=7, sticky=N+S+E+W)

		current_value = Label(root, text="${0:.2f}".format(
                    float(current_value)), bg="white")
		current_value.grid(row=row_count, column=8, sticky=N+S+E+W)

		profit_loss_total = Label(root, text="${0:.2f}".format(
                            float(profit_loss)), bg="silver", fg=red_green(float(profit_loss)))
